...
function save(req, res){
    ...
    Movim.countDocuments({}, function(err, count) {
        ...
        movim.uploadDate = new Date();
        movim.sector = params.sector;
        movim.requester = params.requester;
        movim.ticket = params.ticket;
        movim.depositDestiny = params.depositDestiny;
        movim.company = params.company;
        movim.obs = params.obs;
...
function getList(req, res){
    ...
    Movim.find(query).populate(populateQuery).sort({numMovim: -1}).exec(function(err, list){
        if(err){
            logger.error({message:{module:path.basename(__filename).substring(0, path.basename(__filename).length - 3)+'/'+functionName, msg: decoded.userName+' ('+req.ip+') '+err}});
                res.status(500).send({message: 'Error del servidor en la peticion'})
        }else{
...
function getLastestList(req, res){
    var decoded = jwt_decode(req.headers.authorization);
    var functionName = 'getLastestList';
    var query = req.query;
    var limit = Number.parseInt(req.params.limit);

    Movim.find(query).populate(populateQuery).sort({numMovim: -1}).limit(limit).exec(function(err, list){
        if(err){
            logger.error({message:{module:path.basename(__filename).substring(0, path.basename(__filename).length - 3)+'/'+functionName, msg: decoded.userName+' ('+req.ip+') '+err}});
                res.status(500).send({message: 'Error del servidor en la peticion'})
        }else{
